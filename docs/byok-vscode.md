...
Kestrel/OpenFOAM/AMReX/ReFrame docs, long-term memory, and loaded
skills.

You'll see five models in the VS Code Chat picker, one per `ofa`
mode (OpenFOAM, HPC, Code, AMReX, ReFrame).
...
### 4. Activate the models in VS Code

`Cmd+Shift+P → "Developer: Reload Window"`. Then
`Cmd+Shift+P → "Chat: Manage Language Models"`:

- VS Code may prompt for the apiKey now. Paste the bearer token from
  step 1. (It's stored in your laptop Keychain; editing the JSON later
  does **not** update it — you re-enter via this dialog or by removing
  and re-adding the provider.)
- Click the eye icon on each `OFA · …` row to make it visible.

### 5. Chat

Open Chat. Type "OFA" in the model picker search box to find the five
modes. Pick the one matching your task and chat as normal.
...
| `401 Unauthorized` in VS Code                          | Apikey out of date in Keychain — re-enter via Manage Language Models |
