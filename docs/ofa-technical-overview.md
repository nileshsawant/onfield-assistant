...
- **Interactive CLI** (`ofa`, `ofa --hpc`, `ofa --code`, `ofa --amrex`, `ofa --rhel9_reframe`) — a full agent loop that reads files, executes bash, edits code, and persists session state on Kestrel.
...
| `ofa --amrex` | AMReX/MARBLES dev | `prompts/amrex.txt` | `retrieve_amrex_context` (AMReX + MARBLES source) |
...
| GET | `/v1/models` | bearer | Lists five model IDs (`ofa-openfoam`, `ofa-hpc`, `ofa-code`, `ofa-amrex`, `ofa-reframe`). |
...
| `prompts/amrex.txt` | 8 | AMReX/MARBLES C++ development. |
...
| RAG retrievers (~1770–2580) | `retrieve_context` (OpenFOAM), `retrieve_hpc_context` (Kestrel docs), `retrieve_amrex_context`, `_get_reframe_rag`. Hybrid dense+BM25. |
